...
import MrefSectionAddModal from "./MrefSectionAddModal";
import MrefSectionEditModal from "./MrefSectionEditModal";
import MrefSectionViewModal from "./MrefSectionViewModal";
import { MrefSection as MrefSectionType } from "../types/mrefSection.types";
import {
  useCreateMrefSection,
  useDeleteMrefSection,
  useMrefSections,
...
export default function MrefSection() {
  ...
  const [selected, setSelected] = useState<MrefSectionType | null>(null);
  const [isViewOpen, setIsViewOpen] = useState(false);
  const [isEditOpen, setIsEditOpen] = useState(false);
  const [isAddOpen, setIsAddOpen] = useState(false);
  const [editKey, setEditKey] = useState(0);

  const { data: response, isLoading, isError } = useMrefSections();
  const { mutate: createItem } = useCreateMrefSection();
  ...
  //  view functionality
  const onView = (item: MrefSectionType) => {
    setSelected(item);
    setIsViewOpen(true);
  };

  // edit functionality
  const onEdit = (item: MrefSectionType) => {
    setSelected(item);
  ...
  if (isError || (response && !response.success)) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#F9FAFB]">
        <p className="text-red-500 font-medium">Error loading MREF sections</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6 bg-[#F9FAFB] min-h-screen">
        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody className={cn(isLoading && "opacity-50")}>
              {sections.length > 0 ? (
                sections.map((item: MrefSectionType) => (
                  <TableRow
                    key={item._id}
      ...
      <MrefSectionViewModal
        isOpen={isViewOpen}
        onClose={() => setIsViewOpen(false)}
        section={selected}
      />
      <MrefSectionEditModal
        key={editKey}
        isOpen={isEditOpen}
