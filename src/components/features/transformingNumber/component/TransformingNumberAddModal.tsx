...
import { useCreateTransformingNumber } from "../hooks/useTransformingNumber";
import { toast } from "sonner";
import { TransformingNumberItem } from "../types/transformingNumber.type";

interface TransformingNumberAddModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export default function TransformingNumberAddModal({
  isOpen,
  onClose,
}: TransformingNumberAddModalProps) {
  const [items, setItems] = useState<TransformingNumberItem[]>([
    { order: 1, value: "", label: "" },
  ]);

  const { mutate: createSection, isPending } = useCreateTransformingNumber();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createSection(
      ...
      {
        onSuccess: () => {
          toast.success("Number section added successfully");
          handleClose();
        },
        onError: () => toast.error("Failed to add number section"),
      },
    );
  };
