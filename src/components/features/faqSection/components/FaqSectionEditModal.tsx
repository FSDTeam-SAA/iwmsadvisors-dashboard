...
import { FaqItem, FaqSection } from "../types/faqSection.types";

interface Props {
  isOpen: boolean;
  onClose: () => void;
  section: FaqSection | null;
  onSave: (id: string, data: Partial<FaqSection>) => void;
}
...
export default function FaqSectionEditModal({
  ...
  const [items, setItems] = useState<FaqItem[]>(section?.items || []);
  ...
  const submit = (e: React.FormEvent) => {
    e.preventDefault();
    if (section) {
      onSave(section._id, { title, subtitle, items });
      close();
    }
  };
