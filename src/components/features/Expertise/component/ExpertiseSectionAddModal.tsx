...
interface ExpertiseSectionAddModalProps {
  isOpen: boolean;
  onClose: () => void;
  defaultTitle?: string;
  defaultOrder?: number;
...
export default function ExpertiseSectionAddModal({
  isOpen,
  onClose,
  defaultTitle = "Expertise",
  defaultOrder = 4,
}: ExpertiseSectionAddModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    createSection(payload,
      {
        onSuccess: () => {
          toast.success("Expertise section created successfully");
          onClose();
        },
        onError: (error) => {
          const errorMessage =
            (isAxiosError(error) && error.response?.data?.message) ||
            "Failed to create expertise section";
