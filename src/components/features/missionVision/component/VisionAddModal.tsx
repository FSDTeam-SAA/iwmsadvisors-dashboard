...
import { useCreateVision } from "../hooks/useMissionVision";
import { toast } from "sonner";

interface VisionAddModalProps {
  readonly isOpen: boolean;
  readonly onClose: () => void;
}

export default function VisionAddModal({
  isOpen,
  onClose,
}: VisionAddModalProps) {
  ...
  const { mutate: createVision, isPending } = useCreateVision();
    ...
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-[600px] max-h-[90vh] overflow-y-auto p-0 border-none shadow-2xl">
        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
          ...
          <div className="space-y-2">
            <Label
              htmlFor="add-v-description"
              className="text-sm font-bold text-gray-700"
            >
              Description
            </Label>
            <Textarea
              id="add-v-description"
              value={description}
